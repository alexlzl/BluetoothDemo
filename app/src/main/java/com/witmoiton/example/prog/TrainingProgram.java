...
public class TrainingProgram {
    ...
    public int beforeAngle = 0;              //记录前一个角度
    ...
    public List<Integer> saveAngleStamp = new ArrayList<>();//存储角度时间错
    ...
    public int sensorError = 1;              //静止异常只播放一次
    ...
    public Long wTimer = 0L;                 //检测角度时间，用于静止监控
...
//        整理规则数据
    ...
    */
    ...
    public void recordJointAngle(int jointAngle) {
        ...
        if ((trainingName.equals("K7") && status.equals("中立位抓取"))
        ...
        } else if ((trainingName.equals("K5") && status.equals("勾脚保持"))
                || (trainingName.equals("K6") && status.equals("勾脚保持"))
                || (trainingName.equals("K10") && status.equals("前重心保持"))
        ) {
            if (angleMin == 370) {
                angleMin = jointAngle;
            } else if (jointAngle < angleMin && patientActionStatus == 1) {
                angleMin = jointAngle;
            }
            if (segmentMin == 370) {
                segmentMin = baseAngle;
            } else if (baseAngle < segmentMin && patientActionStatus == 1) {
                segmentMin = baseAngle;
            }
        } else if ((trainingName.equals("K5") && status.equals("踩脚保持"))
                || (trainingName.equals("K6") && status.equals("踩脚保持"))
                || (trainingName.equals("K10") && status.equals("后重心保持"))
        ) {
            if (angleMax == 0) {
                angleMax = baseAngle;
            } else if (jointAngle > angleMax && patientActionStatus == 1) {
                angleMax = jointAngle;
            }
            if (segmentMax == 0) {
                segmentMax = baseAngle;
    ...
    public void responseTimer() {
        if (
                status.equals("第二次静止") || status.equals("标准") || status.equals("计数")
                        ...
                        || status.equals("过高计数") || status.equals("勾脚保持") || status.equals("踩脚保持")
                        || status.equals("后重心保持") || status.equals("前重心保持")
        ) {
            int diff;
            int timerStatus = 2;
    ...
    public void judgeSensorStatic() {
        ...
        if (wTimer == 0) {
            ...
            if (meanAngle1 > 0.05 || meanAngle2 > 0.05) {
                wTimer = new Date().getTime();
            }
        }
        Long currentTime = new Date().getTime();
        double timeDiff = (double) (currentTime - wTimer) / 1000;
        if (timeDiff > 10 && sensorError == 1) {
            sensorError = 0;
            Message reportMsg = mainHandler.obtainMessage(2029);
            reportMsg.arg1 = 1;
            mainHandler.sendMessage(reportMsg);
    ...
    public void process(Map vecs) {
        ...
        int angle1;
        int angle2;
        if (trainingName.equals("K2") || trainingName.equals("K3") || trainingName.equals("K4")) {
            angle1 = (int) Math.round(common.getAngleYZAngle1((Map) vecs.get(thighName)));
            angle2 = (int) Math.round(common.getAngleYZAngle2((Map) vecs.get(shankName),(Map) vecs.get(thighName)));

        } else {
            angle1 = (int) Math.round(common.getAngleY((Map) vecs.get(thighName)));
            angle2 = (int) Math.round(common.getAngleY((Map) vecs.get(shankName)));
        }
    ...
    public int transformUIAngle(int angle) {
        ...
        } else if (trainingName.equals("K29") || trainingName.equals("K30")) {
            if (angle < 90) {
                transAngle = 0;
        ...
        for (int i = 0; i < priorityKeysList.size(); i++) {
            ...
            if (isSatisfyRule((String) rule.get("ruleName"), jointAngle, diffVoice)) {
                ...
                if ((Integer) rule.get("loopPlayVoice") >= 0) {
                    if (!currentVoice1.equals("-") || loopReport != 0) {
                        loopPlayVoice((Integer) rule.get("loopPlayVoice"), currentVoice1);
                    }
                }
            ...
            case 3:
                ...
                if (!Arrays.asList(twiceCountName).contains(this.trainingName)) {
                    if ((nextStatus.equals("结束"))
                            || (!status.equals("勾脚保持") && nextStatus.equals("勾脚保持"))
                            || (!status.equals("踩脚保持") && nextStatus.equals("踩脚保持"))
                            || (!status.equals("后重心保持") && nextStatus.equals("后重心保持"))
                            || (!status.equals("前重心保持") && nextStatus.equals("前重心保持"))
                    ...
                    //补充训练结束的计时器时间
                    if (nextStatus.equals("休息监控") || nextStatus.equals("踩脚开始")
                            || nextStatus.equals("后重心开始")) {
                        Message timerMsge = this.mainHandler.obtainMessage(2024);
                        timerMsge.arg1 = lastTime + 1;
                        timerMsge.arg2 = 2; //timerStatus
                        this.mainHandler.sendMessage(timerMsge);
                ...
                //补充结束休息结束后的计时器
                if (status.equals("结束") && (nextStatus.equals("准备动作检查1") ||
                        nextStatus.equals("准备动作检查"))) {
                    int timerStatus = 1;
...
//            countS.arg1 = onceStatus;
    ...
    语音动作处理
    */
    public void voicePrompt(String currentVoice) {

        if (!currentVoice.equals("-")) {
