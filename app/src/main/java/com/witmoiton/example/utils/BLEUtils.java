...
import com.witmoiton.example.OnDeviceSearchListener;
...
public class BLEUtils {
                ...
                if (success) {
                    ...
                    for(BluetoothGattDescriptor dp: characteristic.getDescriptors()){
                        if (dp != null) {
                            ...
                            boolean isSuccess =  gatt.writeDescriptor(dp);
                            Log.d(TAG,"enableNotification  ==  " + isSuccess);
                        }
                    }
                }
            }
     ...
     * @return
     */
    public boolean writeRXCharacteristic(BluetoothGatt gatt, UUID serviceUUID, UUID characteristicUUID,String value) {
        BluetoothGattService RxService = gatt.getService(serviceUUID);
        if (RxService == null) {
            //Service not supported
            Log.d(TAG, "writeRXCharacteristic: Service not supported");
            return false;
        }
        BluetoothGattCharacteristic RxChar = RxService.getCharacteristic(characteristicUUID);
        if (RxChar == null) {
            Log.d(TAG, "writeRXCharacteristic: Service not supported");
            return false;
        }

        //RxChar.setWriteType(BluetoothGattCharacteristic.WRITE_TYPE_SIGNED);

        RxChar.setValue(HexUtil.hexStringToBytes(value));
